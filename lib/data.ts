...
import { nextStreak, studyStartDate } from '@/lib/progress';
...
export async function getDashboard() {
  ...
  // The curriculum clock starts when the user first studied this course — the
  // earliest focus log or item completion — not when they enrolled. This anchors
  // the calendar-paced week (lib/progress.currentWeekNumber) to real activity.
  const courseStart = studyStartDate(
    (items.data ?? []) as Item[],
    (progress.data ?? []) as Progress[],
    (timeLogs.data ?? []) as TimeLog[],
  );
  return {
    items: items.data ?? [],
...
// derived from that test data, and zero the streak. With no study activity left,
// studyStartDate returns null and currentWeekNumber falls back to the completion-gated
// rule (all items not_started → week 1), so the weekly review reflects only real use
// going forward. Journal entries are kept.
